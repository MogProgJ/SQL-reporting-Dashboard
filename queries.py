...
def get_customer_summary(customer_name: str) -> pd.DataFrame:
    """Summary stats for a single customer."""
    sql = f"""
    SELECT
      c.name                                              AS customer,
      c.segment,
    ...
    GROUP BY c.name, c.segment, c.city;
    """
    return run_query(sql, (customer_name,))


def get_customer_trend(customer_name: str) -> pd.DataFrame:
    """Daily revenue and order count for a single customer."""
    sql = f"""
    SELECT
      o.created_at::date                             AS order_date,
      COALESCE(SUM(oi.quantity * oi.unit_price_cents), 0) AS revenue_cents,
...
def get_customer_products(customer_name: str, limit: int = 10) -> pd.DataFrame:
    """Top products purchased by a single customer."""
    sql = f"""
    SELECT
      p.name                                             AS product,
      cat.name                                           AS category,
    ...
    LIMIT %s;
    """
    return run_query(sql, (customer_name, limit))


def get_customer_orders(customer_name: str) -> pd.DataFrame:
    """All order-item rows for a single customer."""
    sql = f"""
    SELECT
      o.id                                  AS order_id,
      o.created_at::date                    AS order_date,
...
def get_product_summary(product_name: str) -> pd.DataFrame:
    """Summary stats for a single product."""
    sql = f"""
    SELECT
      p.name                                              AS product,
      cat.name                                            AS category,
    ...
    GROUP BY p.name, cat.name;
    """
    return run_query(sql, (product_name,))


def get_product_trend(product_name: str) -> pd.DataFrame:
    """Daily revenue and units sold for a single product."""
    sql = f"""
    SELECT
      o.created_at::date                             AS order_date,
      COALESCE(SUM(oi.quantity * oi.unit_price_cents), 0) AS revenue_cents,
...
def get_product_customers(product_name: str, limit: int = 10) -> pd.DataFrame:
    """Top customers who bought a single product."""
    sql = f"""
    SELECT
      c.name                                             AS customer,
      c.segment,
    ...
    LIMIT %s;
    """
    return run_query(sql, (product_name, limit))


def get_product_orders(product_name: str) -> pd.DataFrame:
    """All order-item rows for a single product."""
    sql = f"""
    SELECT
      o.id                                  AS order_id,
      o.created_at::date                    AS order_date,
